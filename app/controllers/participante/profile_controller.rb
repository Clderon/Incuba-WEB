# app/controllers/participante/profile_controller.rb
class Participante::ProfileController < Participante::BaseController
  ...
  def update_avatar
    if params[:user] && params[:user][:avatar]
      ...
      if @user.update(avatar: params[:user][:avatar])
        redirect_to participante_profile_path, 
  ...
  def set_current_user_as_profile
    # Solo permitir que los participantes vean/editen SU PROPIO perfil
    @user = current_user
    
    # Verificación adicional de seguridad
    unless @user&.es_participante?
  ...
  def handle_password_change
    Rails.logger.info "🔍 DEBUG: handle_password_change iniciado para participante"
    ...
    if @user.update(password: new_password, password_confirmation: password_confirmation)
      Rails.logger.info "✅ DEBUG: Contraseña actualizada exitosamente para participante"
    ...
    if request.format.json?
      ...
      redirect_to participante_profile_path, alert: "Error interno del servidor"
    end
  end
  ...
  def log_password_change
    Rails.logger.info "🔐 CONTRASEÑA CAMBIADA (Participante) - Usuario: #{@user.email}"
  ...
  def refresh_cache_after_user_status_change(user_id)
    ...
    Rails.logger.info "🔄 Caché limpiado después de cambio de estado para participante #{user_id}"
  end
end